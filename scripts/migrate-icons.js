...
function migrateFile(filePath) {
  ...
  // Find which Font Awesome icon is used
  for (const [faIcon, lucideIcon] of Object.entries(iconMapping)) {
    // Check if file imports this icon
    const importRegex = new RegExp(`import\\s*\\{\\s*${faIcon}\\s*}\\s*}\\s*from\\s+['"]@fortawesome/free-solid-svg-icons['"]`);
    if (importRegex.test(content)) {
      ...
      const faIconUsageRegex = new RegExp(`<FontAwesomeIcon\\s+\\n\\s+icon={${faIcon}}\\s+\\n\\s+(?:size="([^"]+)"\\s*)?\\s*/>`, 'g');
      content = content.replace(
        faIconUsageRegex,
        `<${lucideIcon.component} size={${lucideIcon.size}} />`
      );
    }
  }
